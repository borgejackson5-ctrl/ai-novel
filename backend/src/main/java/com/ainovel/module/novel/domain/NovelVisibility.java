...
/**
 * 作品「对外可分发」的判定口径，为全项目唯一实现。
 *
 * <p><b>规则</b>：已上架（status=1）且审核通过（1）或变更待审（3）。
 * 变更待审(3) 属于「已上架、改动待审」，前台展示的仍是旧内容，因此保持可见。
 *
 * <p><b>收敛的必要性</b>：书库分页、搜索全量重建、索引同步、定时对账四条路径均依赖该口径。
 * 若各自实现一份，修改时遗漏其中一处即会产生「搜得到、点进去 404」这类难以排查的脏数据，
 * 而每段代码表面均无异常。
 */
